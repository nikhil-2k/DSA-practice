...
void generateKeyTable(char key[], int ks,   char keyT[5][5])  {  
 ...
 i = 0;  
 j = 0;  
 for (k = 0; k < ks; k++) {   if (dicty[key[k] - 97] == 2) {   dicty[key[k] - 97] -= 1;   keyT[i][j] = key[k];   j++;  
 if (j == 5) {  
 i++;  
 j = 0;  
 }  
...
{  
 ...
 b = 'i';  
 for (i = 0; i < 5; i++) {  
 for (j = 0; j < 5; j++) {   if (keyT[i][j] == a) {   arr[0] = i;  
 arr[1] = j;  
 }  
...
void decrypt(char str[], char keyT[5][5], int ps)  {  
 int i, a[4];  
 for (i = 0; i < ps; i += 2) {   search(keyT, str[i], str[i + 1], a);   if (a[0] == a[2]) { 
